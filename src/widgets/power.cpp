...
bool PowerTextShape::updated(const TelemetryData &data) const {
	if (is_initialized_) {
...
void PowerTextShape::initialize(cairo_t *cr) {
	if (is_initialized_)
		return;
...
void PowerTextShape::clear(void) {
	TextShape::clear();
...
#define ADD_UNIT(unit) \
	...
	ADD_UNIT(TelemetryData::UnitWatt);
