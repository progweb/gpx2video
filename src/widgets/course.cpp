...
bool CourseTextShape::updated(const TelemetryData &data) const {
	if (is_initialized_) {
...
void CourseTextShape::initialize(cairo_t *cr) {
	if (is_initialized_)
		return;
...
void CourseTextShape::draw(cairo_t *cr, const TelemetryData &data) {
	char s[128];
	...
	if (!no_value_)
		sprintf(s, "%d", (int) std::round(data.course()));
	else
		sprintf(s, "--");
...
void CourseTextShape::clear(void) {
	TextShape::clear();
...
#define ADD_UNIT(unit) \
	...
	ADD_UNIT(TelemetryData::UnitDegree);
