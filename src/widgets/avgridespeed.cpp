...
bool AvgRideSpeedTextShape::updated(const TelemetryData &data) const {
	if (is_initialized_) {
...
void AvgRideSpeedTextShape::initialize(cairo_t *cr) {
	if (is_initialized_)
		return;
...
void AvgRideSpeedTextShape::clear(void) {
	TextShape::clear();
...
AvgRideSpeedWidget::AvgRideSpeedWidget(GPXApplication &app, TelemetrySource *source)
	: VideoWidget(app, VideoWidget::WidgetAverageRideSpeed, source) 
