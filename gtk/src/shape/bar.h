#ifndef __GPX2VIDEO__GTK__BARSETTINGSBOX_H__
#define __GPX2VIDEO__GTK__BARSETTINGSBOX_H__
...
class GPX2VideoBarSettingsBox : public GPX2VideoBaseSettingsBox {
public:
	GPX2VideoBarSettingsBox()
		: GPX2VideoBaseSettingsBox("GPX2VideoBarSettingsBox")
		, widget_(NULL) {
	}

	GPX2VideoBarSettingsBox(BaseObjectType *cobject,
		const Glib::RefPtr<Gtk::Builder> &ref_builder, std::string resource_file, GPX2VideoWidget *widget);

	static GPX2VideoBarSettingsBox * create(GPX2VideoWidget *widget) {
		log_call();
		...
		auto box = Gtk::Builder::get_widget_derived<GPX2VideoBarSettingsBox>(ref_builder, "settings_box", resource_file, widget);
	...
	virtual ~GPX2VideoBarSettingsBox() {
		log_call();
	}
