...
void GPX2VideoMapWidgetSettingsBox::load_models(void) {
...

	{
		...
		auto row = *iter;
		row[model_.m_id] = MapSettings::FollowNone;
		row[model_.m_name] = _("None");
		row[model_.m_enable] = true;

		row = *(follow_model_->append());
		row[model_.m_id] = MapSettings::FollowCourse;
		row[model_.m_name] = _("Course");
		row[model_.m_enable] = true;

		row = *(follow_model_->append());
		row[model_.m_id] = MapSettings::FollowHeading;
	...
	combobox->signal_changed().connect(sigc::bind(
				sigc::mem_fun(*this, &GPX2VideoMapWidgetSettingsBox::on_widget_combobox_changed), combobox, 
					[this](const Gtk::TreeModel::const_iterator &iter) {
						...
						((Map *) widget_->widget())->settings().setFollow((MapSettings::Follow) value);
