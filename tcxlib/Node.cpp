...
namespace tcx
{
  Node::Node(Node *parent, const char *name, tcx::Node::Type type, bool mandatory) :
    _parent(parent),
    _name(name),
    _type(type),
    _value(),
  ...
  void Node::copy(Node *source, Report *report)
  {
    ...
    for (std::list<tcx::Node*>::iterator node = source->getAttributes().begin(); node != source->getAttributes().end(); ++node)
    {
      tcx::Node *attribute = add((*node)->getName().c_str(), tcx::Node::ATTRIBUTE, report);

      attribute->copy(*node, report);
    }

    for (std::list<tcx::Node*>::iterator node = source->getElements().begin(); node != source->getElements().end(); ++node)
    {
      tcx::Node *element = add((*node)->getName().c_str(), tcx::Node::ELEMENT, report);
