...
namespace tcx
{
  ...
  void Parser::makeElement(const char *name)
  {
    ...
    if (strcasecmp(name, "TrainingCenterDatabase") == 0)
    {
      if (_root == nullptr)
      {
        _current = _root = new TCX();
        _current->setLine(XML_GetCurrentLineNumber(_parser));
      }
      else if (_report != nullptr)
      {
        _report->report(_root, Report::DOUBLE_TCX, "");
      }
    }
    ...
    {
      _report->report(nullptr, Report::MISSING_TCX, "");
    }
  }
  ...
  void Parser::made()
  {
    ...
    {
      _report->report(nullptr, Report::MISFORMED_TCX, "");
    }
  }
  ...
  TCX *Parser::parse(std::istream &stream)
  {
    bool ok = stream.good();
