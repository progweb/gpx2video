...
//  sc->video_avc = avcodec_find_encoder_by_name(sp.video_codec);
  sc->video_avc = avcodec_find_encoder(decoder_ctx->codec_id);
...
//    av_opt_set(sc->video_avcc->priv_data, sp.codec_priv_key, sp.codec_priv_value, 0);
  ...
  sc->video_avcc->bit_rate = 4 * 1000 * 1000;
  sc->video_avcc->rc_buffer_size = 4 * 1000 * 1000;
  sc->video_avcc->rc_max_rate = 2 * 1000 * 1000;
  sc->video_avcc->rc_min_rate = 8 * 1000 * 1000;

//  A MOI
	sc->video_avcc->framerate = input_framerate;
...
int prepare_audio_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, int sample_rate, StreamingParams sp){
  sc->audio_avs = avformat_new_stream(sc->avfc, NULL);

//  sc->audio_avc = avcodec_find_encoder_by_name(sp.audio_codec);
  sc->audio_avc = avcodec_find_encoder(decoder_ctx->codec_id);
...
//  sc->audio_avcc->time_base      = (AVRational){1, sample_rate};
				  ...
				  sc->audio_avcc->channel_layout = decoder_ctx->channel_layout;
                sc->audio_avcc->channels = av_get_channel_layout_nb_channels(sc->audio_avcc->channel_layout);
                sc->audio_avcc->sample_rate = decoder_ctx->sample_rate;
                sc->audio_avcc->sample_fmt = sc->audio_avc->sample_fmts[0];
...
//    strcat(encoder->filename, sp.output_extension);
  ...
  if (!sp.copy_audio) {
    if (prepare_audio_encoder(encoder, decoder->audio_avcc, decoder->audio_avcc->sample_rate, sp)) {return -1;}
  } else {
  ...
  av_dump_format(encoder->avfc, 0, encoder->filename, 1);
...
//  }
  ...
  if (avformat_write_header(encoder->avfc, NULL /* &muxer_opts */) < 0) {logging("an error occurred when opening output file"); return -1;}
